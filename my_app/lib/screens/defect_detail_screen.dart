...
class _DefectDetailScreenState extends State<DefectDetailScreen> {
  ...
  Future<void> _loadDefect() async {
    ...
    try {
      final defect = await _dataService.getVehicleDefect(widget.defectId);
      if (mounted) {
        setState(() {
          _defect = defect;
          _loading = false;
  ...
  Future<void> _addPhotos() async {
    try {
      ...
      final updated = await _dataService.addDefectImages(widget.defectId, photos);
  ...
  Future<void> _changeStatus() async {
    ...
    try {
      final updated = await _dataService.changeDefectStatus(defect.id, newStatus);
      if (mounted) {
  ...
  Future<void> _addComment(String text) async {
    final updated = await _dataService.addVehicleDefectComment(widget.defectId, text);
    if (mounted) setState(() => _defect = updated);
  }
  ...
  void _openPhoto(int initialIndex) {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => _DefectPhotoViewer(images: defect.images, initialIndex: initialIndex),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          if (widget.canManageVehicles && defect != null)
            TextButton(
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                '${defect.vehicleName} · ${defect.severityLabel} severity',
...
class _DefectPhotoViewer extends StatefulWidget {
  final List<VehicleDefectImage> images;
  final int initialIndex;

  const _DefectPhotoViewer({required this.images, required this.initialIndex});

  @override
  State<_DefectPhotoViewer> createState() => _DefectPhotoViewerState();
}

class _DefectPhotoViewerState extends State<_DefectPhotoViewer> {
